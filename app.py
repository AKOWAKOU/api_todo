...
def update_task(task_id):
    ...
    try:
        # Accéder à la tâche via votre modèle
        ...
        # Validation et mise à jour du champ assigned_to (ID de l'utilisateur)
        if 'assigned_to' in data and ObjectId.is_valid(data.get('assigned_to')):
            user = db.get_users()  # Utilisation de votre méthode pour obtenir les utilisateurs
            user = next((u for u in user if str(u['_id']) == data.get('assigned_to')), None)  # Recherche de l'utilisateur par ID
            
            if not user:
                return jsonify({"error": "Utilisateur assigné non trouvé"}), 404
            
            update_data['assigned_to'] = str(data.get('assigned_to'))

        # Mise à jour de la tâche dans la base de données
        db.tasks.update_one({"_id": ObjectId(task_id)}, {"$set": update_data})

        return jsonify({"msg": "Mise à jour de la tâche réussie"}), 200

    except Exception as e:
        return jsonify({"error": f"Tâche invalide : {str(e)}"}), 400
